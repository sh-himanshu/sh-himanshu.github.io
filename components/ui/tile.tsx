...
export function Tile({
  ...
  const syncPointerPosition = () => {
    ...
    tileRef.current.style.setProperty("--mouse-x", `${pointerRef.current.x}px`);
    tileRef.current.style.setProperty("--mouse-y", `${pointerRef.current.y}px`);
  ...
  const handlePointerMove = (event: PointerEvent<HTMLElement>) => {
    ...
    if (frameRef.current === null) {
      frameRef.current = window.requestAnimationFrame(syncPointerPosition);
    }
  };
